...
hero("📐 Mô hình OLS", "Ước lượng hồi quy tuyến tính đa biến và kiểm định mô hình", ["OLS", "Regression", "P-value"])
...
model = fit_ols(df)
summary = ols_summary_table(model)

c1, c2, c3, c4 = st.columns(4)
with c1:
    metric_card("R²", f"{model.rsquared:.3f}", "✅")
with c2:
    metric_card("Adjusted R²", f"{model.rsquared_adj:.3f}", "📌")
with c3:
    metric_card("F p-value", f"{model.f_pvalue:.4f}", "🧪")
with c4:
    metric_card("Quan sát", f"{int(model.nobs)}", "📊")

section("Bảng hệ số hồi quy")
...
st.plotly_chart(residual_chart(report), use_container_width=True)

with st.expander("Xem tóm tắt kỹ thuật từ statsmodels"):
