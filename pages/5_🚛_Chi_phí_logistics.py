...
hero("🚛 Ước tính chi phí logistics", "Tính chi phí nhiên liệu và mô phỏng kịch bản tăng giá dầu", ["Fleet", "Fuel Cost", "Scenario"])
show_banner("logistics_truck.jpg")
...
with v4:
    days = st.number_input("Ngày vận hành/tháng", min_value=1, value=26)

monthly_liters = vehicles * km_day * liter_per_km * days
monthly_cost = monthly_liters * price

m1, m2, m3 = st.columns(3)
m1.metric("Giá dầu dự báo", f"{price:,.0f} VNĐ/lít")
m2.metric("Nhiên liệu/tháng", f"{monthly_liters:,.0f} lít")
m3.metric("Chi phí/tháng", f"{monthly_cost:,.0f} VNĐ")

scenario = pd.DataFrame({
    "Kịch bản": ["Hiện tại", "Tăng 5%", "Tăng 10%", "Tăng 15%", "Tăng 20%"],
    "Giá dầu": [price, price*1.05, price*1.10, price*1.15, price*1.20],
})
scenario["Chi phí tháng"] = scenario["Giá dầu"] * monthly_liters
scenario["Chi phí tăng thêm"] = scenario["Chi phí tháng"] - monthly_cost

section("Kịch bản biến động chi phí")
st.dataframe(scenario.round(0), use_container_width=True, hide_index=True)
st.plotly_chart(bar_chart(scenario, "Kịch bản", "Chi phí tháng", "Chi phí nhiên liệu theo kịch bản"), use_container_width=True)
